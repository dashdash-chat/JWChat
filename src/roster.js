...
function RosterOpenMessage(jid) {
  ...
  if (user.messages.length > 0 && (!user.mW || user.mW.closed)) // display messages
    user.mW = open('message.html?jid='+jid,"mw"+wName,'width=320,height=230,dependent=yes,resizable=yes');
  else if (!user.sW || user.sW.closed) // open send dialog
    user.sW = open("send.html?jid="+jid,"sw"+wName,'width=320,height=200,dependent=yes,resizable=yes');
  return false;
}
